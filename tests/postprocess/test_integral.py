...
def test_short_cycles():
    ...
    example_data = BatteryDataset.make_cell_dataset(
    ...
    assert np.isnan(example_data.tables['cycle_stats']['capacity_charge']).all()
...
def test_capacity(file_path, from_charged):
    ...
    soc.enhance(raw_data)

    assert all(c in raw_data for c in soc.column_names)
    assert not any(raw_data[c].isna().any() for c in soc.column_names)
...
def test_against_battery_data_gov(file_path):
    ...
    orig_data = \
        data.tables['cycle_stats'][
            ['capacity_discharge', 'capacity_charge', 'energy_discharge', 'energy_charge']
        ].copy().iloc[cyc_id]

    # Recompute
    CapacityPerCycle().compute_features(data)
    new_data = data.tables['cycle_stats'][
        ['capacity_discharge', 'capacity_charge', 'energy_discharge', 'energy_charge']].iloc[cyc_id]
...
def test_reuse_integrals(file_path):
    ...
    initial_data = example_data.tables['cycle_stats'][
